...
//!
//! The indexer's signing identity is an Ed25519 key derived deterministically from
//! the dstack TEE seed under purpose `attestmesh.indexer.signing.v1`. It is NOT
//! sealed-stored — it is re-derived every boot. At boot the indexer also requests a
//! TEE quote whose report_data commits to the signing pubkey; that quote is attached
//! to the first push of each subscriber session (spec §6.1, §8.2 step 5).
