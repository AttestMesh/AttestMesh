...
pub async fn launch(
    ...
    if !learned_keys.is_empty() {
        tracing::info!(peers = learned_keys.len(), "loaded persisted peer Ed25519 keys");
    }
        ...
        match envelopes::classify_internal(&pt) {
            Some(pe) => {
                ...
                if ctx
                    ...
                    .lock()
                    .await
                    .set_ed25519(&sender, pe.ed25519_pub)
                {
...
mod tests {
    ...
    fn resend_backoff_doubles_and_caps_at_a_day() {
        ...
        assert_eq!(resend_delay_ms(u32::MAX), day, "no overflow at extreme attempts");
    }
}
