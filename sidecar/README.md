...
- `src/transport/punch.rs` — UDP hole-punch link upgrader ([`docs/specs/udp-transport-upgrade.md`](../docs/specs/udp-transport-upgrade.md)): per-peer `tcp → punching → udp` state machine with automatic revert, peer-to-peer negotiation over `PeerControl`, and the UDP-path watchdog. Gateway TCP stays the bootstrap path and permanent fallback. `/healthz.punch_peer_status` and `attestmesh_peer_punch_status` expose why a peer is bootstrapping, backing off, reverted, unsupported, disabled, punching, or on UDP. Knobs: `WG_UDP_PUNCH` (default `true`), `PUNCH_TIMEOUT_SECS` (10), `PUNCH_RETRY_BACKOFF_SECS` (30, doubling, cap 3600).
