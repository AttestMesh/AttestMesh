...
- [ ] An `IAttestorFacet` convention every attestor facet implements on its stateless
...
- [ ] `ClusterCut` v2: core cuts stay fixed; attestor cuts are built dynamically from
      each configured facet's `selectorManifest()` (staticcall against the
      implementation address, not the diamond).
- [ ] `ClusterDiamondFactory` v2: deploy takes `AttestorConfig[] { address facet,
...
- [ ] `OperatorFacet`: `operator_register(OperatorProof, address memberContract,
...
- [ ] OperatorProof binds exactly what dstack binds: signature by an allowlisted
...
- [ ] Replay safety: a `(memberContract, attestorId)` pair registers at most once
      (already enforced by memberId uniqueness in `_addMember`); the `expiry` bounds
      voucher lifetime.
- [ ] ClusterMember bootstrap validation generalized: in owner-unset mode,
...
- [ ] Sidecar `AttestationProvider` trait with two implementations (dstack wraps the
...
- [ ] Gas-webhook allowlist partitioned per attestor (selector sets keyed by method),
...
- [ ] v1 dstack-only clusters keep working unchanged; adding OperatorFacet to an
      existing cluster is an explicit owner `diamondCut` + `initAttestor` runbook step,
      never automatic.
- [ ] Every user-facing artifact (specs, READMEs, docs site page) that mentions
      OperatorFacet states the trust-model difference explicitly.

### Should Have

- [ ] A `mesh-voucher` helper (CLI or deploy/ routine) for operators to mint
...
- [ ] `AttestFacet.memberAttestorOf(memberId)` view (it's already in storage) so
      off-chain consumers can filter members by admission method.
