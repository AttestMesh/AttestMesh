...
- **DNS-pinning caveat:** the firewall resolves `LLM_BASE_URL`'s host at config time and re-resolves periodically (or pins redpill's published IPs). Documented in the agent README; the firewall script owns this.
- **In-TEE verifiability — the agent self-checks (fail closed):** because the CVM is a sealed TEE (no exec; the box cannot fetch per-container logs), the agent itself TCP-probes a non-allowlisted canary (`EGRESS_CANARY`, default `1.1.1.1:443`) at startup; if reachable, egress is **not** locked → it logs `CRITICAL` and **exits** (refuses to serve). The result (`egress_locked`) is folded into `/healthz`, which is exposed on the box loopback (like the sidecar), so `deploy/matrix-node.sh <node> verify-agent` confirms bootstrap **and** egress lock in one curl. `EGRESS_SELFCHECK=off` skips it (local harness only — no firewall there).
