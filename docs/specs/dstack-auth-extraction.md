...
## Open Questions
- [x] **Proof format validated against dstackgres `group_auth.rs` (2026-06-03).** The real dstack `/GetKey` returns `signature_chain = [app_signature, kms_signature]`; the app key signs `"ethereum:" + hex(derivedCompressedPubkey)`; `codeId = bytes20(app_id)` (app_id from `/Info`); the KMS preimage is `"dstack-kms-issued:" || bytes20(codeId) || appCompressedPubkey`. **All match AttestMesh's ported on-chain `DstackSigChain`** — so the contracts already deployed to Base mainnet will verify a real proof. A live capture remains the final field-for-field confirmation (only obtainable from a booted CVM).
- [x] **Confirmed: `purpose = "ethereum"`** (was the placeholder `"app-key"`). `MockKmsChain.PURPOSE` updated. The sidecar's KMS-chain method will set `purpose="ethereum"` and map `signature_chain[0]→appSignature`, `[1]→kmsSignature`.
