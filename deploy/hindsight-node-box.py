#!/usr/bin/env python3
"""Box-side deploy helper for deploy/hindsight-node.sh.
...
Three modes:
  ...
  update <app_id> <vm_id>
                  — roll a new compose/env onto an existing app. Default = IN-PLACE,
                    ...
                    so the pg0 memory store survives; the compose_hash change is only an
                    on-chain KMS auth gate, not part of the disk key. With BOX_FRESH_DISK=1
                    (a DELIBERATE wipe) it does a fresh-disk CreateVm.
...
pre_launch_script + APP_ID in allowed_envs); keep in sync with synclave-node-box.py.

  sudo E_RPC_URL=… … BOX_COMPOSE=/tmp/hindsight-node.yaml \
    /opt/dstack-mcp/venv/bin/python hindsight-node-box.py deploy
"""
...
NAME = os.environ.get("BOX_NAME", "hindsight-node")
COMPOSE_PATH = os.environ.get("BOX_COMPOSE", "/tmp/hindsight-node.yaml")
VCPU = int(os.environ.get("BOX_VCPU", "4"))
MEM = int(os.environ.get("BOX_MEM", "6144"))
DISK = int(os.environ.get("BOX_DISK", "40"))
PORTS = json.loads(os.environ.get("BOX_PORTS", "[]"))
# CVM networking: "bridge" (default) → routable on dstack-br0. In bridge mode KMS
# is the SLIRP alias 10.0.2.2 (RA-TLS cert SAN), reached via the host DNAT.
NET_MODE = (os.environ.get("BOX_NET_MODE", "bridge").strip().lower() or "bridge")
# Gateway ON for the sidecar's wg-over-gateway-TCP mesh ingress on 51900
# (<app_id>-51900s). Hindsight's HTTP surface is NOT compose-published, so the
# gateway has nothing else to route. Measured into compose_hash.
...
ENV_KEYS = [
    ...
    "GATEWAY_DOMAIN",
    # --- Hindsight (key NAMES measured into compose_hash, VALUES sealed;
...
def main() -> None:
    ...
    if mode == "update":
        ...
        if not app_id or not vm_id:
            raise SystemExit("usage: hindsight-node-box.py update <app_id> <vm_id>")
