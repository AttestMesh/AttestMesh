#!/usr/bin/env python3
"""Box-side deploy helper for deploy/hermes-node.sh.

Runs on the self-hosted dstack box. It deploys a Hermes agent AttestMesh node
(deploy/compose/hermes-node.yaml): sidecar + two workbench shells + the Hermes
gateway + Paseo, with the agent's per-identity secrets (Matrix token, Hindsight
key, Fastmail creds, GitHub PAT, model key) sealed into the CVM. The app is a
stock DstackApp at first; deploy/hermes-node.sh primes the cluster and upgrades
the app proxy to ClusterMember (Path A).
"""
...
NAME = os.environ.get("BOX_NAME", "hermes-node")
COMPOSE_PATH = os.environ.get("BOX_COMPOSE", "/tmp/hermes-node.yaml")
...
# Optional keys (EMAIL_*, GITHUB_*, WORKSPACE_GIT_URL, …) may be unset → "".
ENV_KEYS = [
    "CHAIN_ID",
    "RPC_URL",
    "BUNDLER_URL",
...
def main() -> None:
    ...
    if mode == "update":
        ...
        if not app_id or not vm_id:
            raise SystemExit("usage: hermes-node-box.py update <app_id> <vm_id>")
    ...
    raise SystemExit("usage: hermes-node-box.py [deploy|hash|update <app_id> <vm_id>]")
