#!/usr/bin/env bash
# ClickHouse HA cluster deploy on the self-hosted dstack box: N nodes (default 3) of
# ClickHouse Keeper + clickhouse-server + HAProxy, joined to the Matrix node's AttestMesh
# cluster (C3). Structural copy of deploy/pg-ha-node.sh / deploy/redis-ha-node.sh (same
# 3-phase chain split); order-sensitive, logged, re-entrant via state files.
#
# CVM deployment is split register -> compute-peers -> create so every node's mesh IP is
# derived off-chain (AttestFacet.meshIpOf math) BEFORE any CVM boots, giving the Keeper
# raft a static CH_PEERS ensemble with zero off-chain coordination.
set -uo pipefail
...
NODE="${1:?usage: clickhouse-ha-node.sh <name> [register-all|compute-peers|create-all|prime|bind|verify|verify-ha|verify-failover|verify-isolation|update <chN>|update-all|all]}"
ACTION="${2:-all}"
ARG3="${3:-}"
...
COMPOSE="${COMPOSE:-$ROOT/deploy/compose/clickhouse-ha-node.yaml}"
...
CHHA_COUNT="${CHHA_COUNT:-3}"

# Langfuse analytics is the main tenant: more memory + disk than redis-ha. Mesh-only:
...
[ $((CHHA_COUNT % 2)) -eq 0 ] && log "⚠ CHHA_COUNT=$CHHA_COUNT is EVEN — keeper raft wants 3 or 5"

_nodes() { local i; for i in $(seq 1 "$CHHA_COUNT"); do echo "ch$i"; done; }
_node_state() { echo "$LOGDIR/clickhouse-ha-node-${NODE}-$1.state"; }
...
CHHA_INITIALIZED=${CHHA_INITIALIZED:-}
EOF
}
...
# clickhouse-ha joins the existing Matrix cluster (C3) by default.
...
_box_run() {
  local mode="$1" node="${2:-ch1}" app_id="${3:-}" vm_id="${4:-}" guser gtok bootstrap
  ...
  scp -o BatchMode=yes -q "$HERE/clickhouse-ha-node-box.py" "$BOX_HOST:/tmp/clickhouse-ha-node-box.py"
  {
    printf 'E_CHAIN_ID=%q\n'              "$CHAIN_ID"
    printf 'E_RPC_URL=%q\n'               "${CVM_RPC_URL:-$RPC_URL}"
    ...
    bash -c 'set -a; . /dev/stdin; set +a; exec $BOX_PY /tmp/clickhouse-ha-node-box.py $mode $app_id $vm_id'"
}
...
compute_peers() {
  ...
  log "reserved (non-chha) member IPs:$used"
  CH_PEERS=""
  for n in $(_nodes); do
    _nload "$n"
    ...
    MESH_IP="$ip"
    _nsave "$n"
    CH_PEERS="${CH_PEERS:+$CH_PEERS,}$n=$ip"
    log "✔ $n → memberId=$mid meshIp=$ip"
  done
  _save_cluster
  log "✔ CH_PEERS=$CH_PEERS (mesh $MESH_CIDR_STR)"
}

create_all() {
  _load_cluster; _default_cluster_env; _require_env; _mesh_math_init
  [ -n "${CH_PEERS:-}" ] || die "no CH_PEERS — run compute-peers first"
  CHHA_VERIFY_PASSWORD="${CHHA_VERIFY_PASSWORD:-$(openssl rand -hex 24)}"
  _save_cluster
  local n out j bootstrap
  bootstrap=new
  [ -n "${CHHA_INITIALIZED:-}" ] && bootstrap=join   # scale-out onto a live cluster
  for n in $(_nodes); do
    _nload "$n"
    [ -n "$X" ] || die "$n has no app_id — run register-all first"
...
prime_all() {
  ...
  if [ "$allowed" = true ]; then
    ...
    send_seq "chha-addHash-${NODE}" "$CLUSTER" "addComposeHash(bytes32)" "0x${h#0x}"
  fi
  for n in $(_nodes); do
    _nload "$n"
    ...
    if [ "$allowed" = true ]; then
      ...
      send_seq "chha-addApp-${NODE}-${n}" "$CLUSTER" "addAllowedAppId(address)" "$X"
    fi
  done
}
...
bind_all() {
  ...
  for n in $(_nodes); do
    ...
    ssh_box "sudo bash -s" <<SCRIPT 2>&1 | tee "$LOGDIR/chha-bind-${NODE}-${n}.$(ts).log"
...
SCRIPT
    confirm_latest_transaction "chha-bind-${NODE}-${n}" "$RPC_URL" "$LOGDIR/chha-bind-${NODE}-${n}.*.log" || die "bind transaction not confirmed for $n"
    c=""
...
verify_all() {
  ...
  for n in $(_nodes); do
    ...
    # The keeper raft bootstrapped with the PRECOMPUTED IP — a mismatch here means the
...
_mesh_ssh() {
  ...
  ssh -o BatchMode=yes -o ConnectTimeout=15 -o StrictHostKeyChecking=accept-new \
      -o UserKnownHostsFile="$LOGDIR/chha-mesh-shell.known_hosts" \
...
verify_ha() {
  _load_cluster
  [ -n "${CH_PEERS:-}" ] && [ -n "${CHHA_VERIFY_PASSWORD:-}" ] || die "need CH_PEERS + CHHA_VERIFY_PASSWORD in $CSTATE"
  log "▶ HA verification from the mesh shell (peers: $CH_PEERS)"
  {
    printf 'PEERS=%q\nVPW=%q\nEXPECT=%q\n' "$CH_PEERS" "$CHHA_VERIFY_PASSWORD" "$CHHA_COUNT"
    cat <<'RSCRIPT'
set -u
...
command -v curl >/dev/null 2>&1 || { echo "HA: FAIL - curl unavailable on the mesh shell"; exit 9; }
# All queries go over the HTTP interface (:8123 via HAProxy) as the meshverify user.
chq() { curl -fsS --max-time 10 "http://$1:8123/" -u "meshverify:$VPW" --data-binary "$2" 2>/dev/null; }
declare -A IP; NAMES=()
IFS=',' read -ra PAIRS <<< "$PEERS"
for p in "${PAIRS[@]}"; do n="${p%%=*}"; IP[$n]="${p#*=}"; NAMES+=("$n"); done
FIRST="${IP[${NAMES[0]}]}"

echo "HA: waiting for /ping + $EXPECT replicas in system.clusters"
ok=""
for i in $(seq 1 60); do
  reps=$(chq "$FIRST" "SELECT count() FROM system.clusters WHERE cluster = 'default'" || echo 0)
  if [ "${reps:-0}" = "$EXPECT" ]; then ok=1; break; fi
  echo "  … ($i/60) system.clusters replicas=${reps:-?}"
  sleep 15
done
[ -n "$ok" ] || { echo "HA: FAIL - cluster never showed $EXPECT replicas"; exit 2; }
chq "$FIRST" "SELECT cluster, shard_num, replica_num, host_address FROM system.clusters WHERE cluster = 'default' FORMAT TSV"

# Keeper ensemble: one leader + followers, checked with the mntr four-letter word on :9181.
leaders=0; followers=0
for n in "${NAMES[@]}"; do
  mode=$( (exec 3<>"/dev/tcp/${IP[$n]}/9181" && printf 'mntr' >&3 && timeout 5 cat <&3) 2>/dev/null \
    | grep -E '^zk_server_state' | awk '{print $2}')
  echo "  $n keeper state=${mode:-?}"
  [ "$mode" = leader ] && leaders=$((leaders + 1))
  [ "$mode" = follower ] && followers=$((followers + 1))
done
[ "$leaders" = 1 ] && [ "$followers" -ge $((EXPECT - 1)) ] \
  || { echo "HA: FAIL - keeper ensemble unhealthy (leaders=$leaders followers=$followers)"; exit 3; }
echo "HA: keeper raft healthy (1 leader + $followers followers)"

# HAProxy /ping on every node's mesh :8123.
for n in "${NAMES[@]}"; do
  p=$(curl -fsS --max-time 5 "http://${IP[$n]}:8123/ping" 2>/dev/null)
  [ "$(echo "$p" | tr -d '[:space:]')" = "Ok." ] || { echo "HA: FAIL - $n :8123 /ping failed"; exit 5; }
done
echo "HA: HAProxy :8123 healthy on every node"

# Replicated write: create ON CLUSTER, insert via one node, read on all.
stamp="smoke-$(date -u +%s)-$RANDOM"
chq "$FIRST" "CREATE DATABASE IF NOT EXISTS verify ON CLUSTER default" >/dev/null \
  || { echo "HA: FAIL - CREATE DATABASE ON CLUSTER failed"; exit 6; }
chq "$FIRST" "CREATE TABLE IF NOT EXISTS verify.smoke ON CLUSTER default (stamp String, at DateTime DEFAULT now()) ENGINE = ReplicatedMergeTree ORDER BY stamp" >/dev/null \
  || { echo "HA: FAIL - CREATE TABLE ON CLUSTER failed"; exit 6; }
chq "$FIRST" "INSERT INTO verify.smoke (stamp) VALUES ('$stamp')" >/dev/null \
  || { echo "HA: FAIL - insert via :8123 failed"; exit 7; }
for n in "${NAMES[@]}"; do
  seen=""
  for i in $(seq 1 20); do
    got=$(chq "${IP[$n]}" "SELECT count() FROM verify.smoke WHERE stamp = '$stamp'")
    [ "$got" = 1 ] && { seen=1; break; }
    sleep 3
  done
  [ -n "$seen" ] || { echo "HA: FAIL - write did not replicate to $n within 60s"; exit 8; }
done
echo "HA: PASS - ReplicatedMergeTree write visible on every node"
RSCRIPT
  } | _mesh_ssh "bash -s" 2>&1 | tee "$LOGDIR/chha-ha-${NODE}.$(ts).log"
  local rc=${PIPESTATUS[1]}
  [ "$rc" = 0 ] || die "HA verification failed (rc=$rc)"
  _load_cluster
  if [ -z "${CHHA_INITIALIZED:-}" ]; then
    CHHA_INITIALIZED=1
    _save_cluster
...
# Replica-loss drill: stop ONE replica (never the keeper leader's quorum majority — with
# 3 nodes any single loss keeps quorum), prove writes continue via the survivors' HAProxy,
# restart it, and watch system.replicas lag drain.
verify_failover() {
  _load_cluster
  [ -n "${CH_PEERS:-}" ] && [ -n "${CHHA_VERIFY_PASSWORD:-}" ] || die "need CH_PEERS + CHHA_VERIFY_PASSWORD"
  local victim="${ARG3:-ch2}"
  _nload "$victim"
  [ -n "$VM_ID" ] || die "no VM_ID recorded for $victim"
  log "▶ replica-loss drill: stopping $victim (vm $VM_ID)"
  _box_run stop "$victim" "" "$VM_ID" >/dev/null || die "StopVm failed"
  {
    printf 'PEERS=%q\nVPW=%q\nVICTIM=%q\n' "$CH_PEERS" "$CHHA_VERIFY_PASSWORD" "$victim"
    cat <<'RSCRIPT'
set -u
chq() { curl -fsS --max-time 10 "http://$1:8123/" -u "meshverify:$VPW" --data-binary "$2" 2>/dev/null; }
declare -A IP; NAMES=()
IFS=',' read -ra PAIRS <<< "$PEERS"
for p in "${PAIRS[@]}"; do n="${p%%=*}"; IP[$n]="${p#*=}"; NAMES+=("$n"); done
stamp="failover-$(date -u +%s)-$RANDOM"
ok=""
for n in "${NAMES[@]}"; do
  [ "$n" = "$VICTIM" ] && continue
  for i in $(seq 1 12); do
    if chq "${IP[$n]}" "INSERT INTO verify.smoke (stamp) VALUES ('$stamp-$n')" >/dev/null 2>&1; then
      ok=1; echo "FAILOVER: write via $n OK with $VICTIM down"; break
    fi
    sleep 5
  done
  [ -n "$ok" ] || { echo "FAILOVER: FAIL - writes via $n did not succeed with $VICTIM down"; exit 3; }
  ok=""
done
echo "FAILOVER: PASS - writes continued through the surviving nodes"
RSCRIPT
  } | _mesh_ssh "bash -s" 2>&1 | tee "$LOGDIR/chha-failover-${NODE}.$(ts).log"
  local rc=${PIPESTATUS[1]}
  log "▶ restarting $victim vm $VM_ID"
  _box_run start "$victim" "" "$VM_ID" >/dev/null || die "StartVm failed (cluster is running degraded on $((CHHA_COUNT - 1)) nodes!)"
  [ "$rc" = 0 ] || die "replica-loss drill failed (rc=$rc) — $victim restarted"
  # Confirm the replica catches up: absolute_delay drains + the drill rows appear on it.
  {
    printf 'PEERS=%q\nVPW=%q\nVICTIM=%q\n' "$CH_PEERS" "$CHHA_VERIFY_PASSWORD" "$victim"
    cat <<'RSCRIPT'
set -u
...
VIP="${IP[$VICTIM]}"
for i in $(seq 1 60); do
  ra=$(chq "$VIP" "SELECT count() FROM system.replicas WHERE is_readonly = 0 AND absolute_delay < 10" 2>/dev/null)
  rows=$(chq "$VIP" "SELECT count() FROM verify.smoke WHERE stamp LIKE 'failover-%'" 2>/dev/null)
  if [ -n "$ra" ] && [ "${ra:-0}" -ge 1 ] && [ "${rows:-0}" -ge 1 ]; then
    echo "REJOIN: PASS - $VICTIM caught up (replicas healthy, drill rows present)"
    exit 0
  fi
  echo "  … ($i/60) $VICTIM healthy_replicas=${ra:-?} drill_rows=${rows:-?}"
  sleep 10
done
echo "REJOIN: FAIL - $VICTIM did not catch up within 600s"
exit 4
RSCRIPT
  } | _mesh_ssh "bash -s" 2>&1 | tee -a "$LOGDIR/chha-failover-${NODE}.$(ts).log"
  rc=${PIPESTATUS[1]}
  [ "$rc" = 0 ] || die "$victim did not catch up (rc=$rc)"
  log "✔ replica-loss drill passed: survivor writes + rejoin + lag drained"
}
...
verify_isolation_all() {
  ...
  for n in $(_nodes); do
    ...
    ssh_box "sudo bash -s" <<SCRIPT 2>&1 | tee "$LOGDIR/chha-isolation-${NODE}-${n}.$(ts).log"
set -u
VMID="$VM_ID"
...
bad=0
for p in 8123 9000 9181 9234 18123 19001 8009; do
...
SCRIPT
  ...
  log "✔ host-isolation invariant holds on all $CHHA_COUNT nodes"
}

# ── day-2 ────────────────────────────────────────────────────────────────────────────────

update_member() {
  local n="${1:?usage: clickhouse-ha-node.sh <name> update <chN>}"
  _load_cluster; _default_cluster_env; _require_env; _mesh_math_init
  [ -n "${CH_PEERS:-}" ] || die "need CH_PEERS in $CSTATE"
  _nload "$n"
  ...
  if [ "$allowed" = true ]; then
    ...
    send_seq "chha-update-addHash-${NODE}" "$CLUSTER" "addComposeHash(bytes32)" "0x$nh"
  fi
  # BOOTSTRAP=join is safe on every roll: preserved data short-circuits it, and a fresh
  # disk (BOX_FRESH_DISK=1) must re-join the established replication/raft anyway.
  ...
  if [ "$mode" = createvm ]; then
    log "✔ $n fresh-disk CreateVm: vm=$VM_ID — membership + mesh IP kept, local data reset (replica will re-sync)"
  else
...
update_all() {
  ...
  # keeper quorum is never at risk.
  local n
  for n in $(_nodes); do
    update_member "$n"
    verify_ha
  done
  log "✔ rolled all $CHHA_COUNT nodes"
}
