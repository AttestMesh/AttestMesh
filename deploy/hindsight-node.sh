...
# Preserve explicitly supplied recovery controls across the production env
# import.  The production file deliberately seals these switches at zero, but
# an operator-authorized one-shot roll must be able to override that value for
...
_CALLER_AUTH_TOKEN_VALUE="${HINDSIGHT_PROVIDER_AUTH_RESET_TOKEN-}"
_CALLER_LLM_CONCURRENCY_SET="${HINDSIGHT_LLM_MAX_CONCURRENT+x}"
_CALLER_LLM_CONCURRENCY_VALUE="${HINDSIGHT_LLM_MAX_CONCURRENT-}"
...
if [ "$_CALLER_AUTH_TOKEN_SET" = x ]; then
  HINDSIGHT_PROVIDER_AUTH_RESET_TOKEN="$_CALLER_AUTH_TOKEN_VALUE"
fi
if [ "$_CALLER_LLM_CONCURRENCY_SET" = x ]; then
...
# Permanent recovery ceiling. Hindsight may fan one retained document into
# multiple provider requests; Agent Sessions remains independently max-one.
HINDSIGHT_LLM_MAX_CONCURRENT="${HINDSIGHT_LLM_MAX_CONCURRENT:-3}"
...
_require_env() {
  ...
  HINDSIGHT_CVM_RPC_URL="${HINDSIGHT_CVM_RPC_URL:-$(box_local_rpc_url "$BOX_HOST" hindsight)}"
...
_box_run() {
  ...
  ssh_box "sudo BOX_NAME='$NODE' BOX_COMPOSE='/tmp/${NODE}.yaml' BOX_VCPU=$BOX_VCPU BOX_MEM=$BOX_MEM BOX_DISK=$BOX_DISK BOX_PORTS='$BOX_PORTS' BOX_GATEWAY_ENABLED='$BOX_GATEWAY_ENABLED' BOX_NET_MODE='$BOX_NET_MODE' BOX_FRESH_DISK='${BOX_FRESH_DISK:-}' \
    E_CHAIN_ID='$CHAIN_ID' E_RPC_URL='$HINDSIGHT_CVM_RPC_URL' E_BUNDLER_URL='${CVM_BUNDLER_URL:-${BUNDLER_URL:-$RPC_URL}}' E_GAS_POLICY_ID='${GAS_POLICY_ID:-}' E_INDEXER_REGISTRY_ADDR='$INDEXER_REGISTRY_ADDR' E_GATEWAY_DOMAIN='$GATEWAY_DOMAIN' \
...
SCRIPT
  confirm_latest_transaction "hindsight-bind-${NODE}" "$RPC_URL" "$LOGDIR/hindsight-bind-${NODE}.*.log" || die "bind transaction not confirmed"
  local c=""
