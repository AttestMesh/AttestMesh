#!/usr/bin/env bash
# pocket-mcp app-tier AttestMesh node on the self-hosted dstack box.
# Poller (2-min loop) + MCP server -> external HA Postgres+pgvector + Hindsight, with
# Qwen embeddings via Redpill. Path-A trio: this driver seals each
# secret as an individual measured env; box helper = pocket-mcp-node-box.py; mesh wiring =
# compose/pocket-mcp-node.yaml.
set -Eeuo pipefail
...
NODE="${1:?usage: pocket-mcp-node.sh <node-name> [deploy|prime|bind|verify|update|all]}"
ACTION="${2:-all}"
...
COMPOSE="${COMPOSE:-$ROOT/deploy/compose/pocket-mcp-node.yaml}"
...
# Gateway remains on for normal bridge egress and mesh connectivity.
...
# Auto-create the app-role DB password on first run (stable across rolls). MCP auth is
# OFF by default (mesh membership is the trust boundary in v1); set MCP_AUTH_TOKEN in the
# secrets file to enforce it.
...
# measured Hindsight state rather than copying the provider key into this workload.
...
# Embeddings use the same metered proxy as Hindsight during the guarded cutover.
# The endpoint returns 4096 dims; the app truncates+renormalizes to EMBEDDING_DIM.
...
HINDSIGHT_BANK="${HINDSIGHT_BANK:-dans-pocket}"
...
# Pocket API plus Redpill embeddings are the only external egress destinations.
if [ -z "${POCKET_API_KEY:-}" ]; then
  [ -s "$POCKET_KEY_FILE" ] || die "no POCKET_API_KEY and no Pocket key at $POCKET_KEY_FILE"
  POCKET_API_KEY="$(tr -d '[:space:]' < "$POCKET_KEY_FILE")"
fi
POCKET_API_BASE="${POCKET_API_BASE:-https://public.heypocketai.com/api/v1}"
# Host-based allowlisting is primary: the egress-fw extracts `public.heypocketai.com` from
# ALLOW_BASE_URL and re-resolves it every RERESOLVE_SECONDS (30s) — handles AWS EIP rotation.
# The two /32s are today's resolved EIPs (us-east-2), pinned as a belt-and-suspenders floor;
# they may drift, but ALLOW_BASE_URL re-resolution covers new addresses.
POCKET_API_CIDRS="${POCKET_API_CIDRS:-3.19.247.64/32 3.21.239.100/32}"

# App DB DSN: multi-host over the sidecar-netns forwarders; survives pg-ha rolls/failover.
...
STATE="$LOGDIR/pocket-mcp-node-${NODE}.state"
...
_require_env() {
  ...
  CVM_RUNTIME_RPC_URL="${CVM_RUNTIME_RPC_URL:-$(box_local_rpc_url "$BOX_HOST" hindsight)}"
  ...
  case "$POCKET_API_BASE" in
    REPLACE_WITH_*|"") die "set POCKET_API_BASE (the external Pocket API host the egress firewall allows on :443)";;
  esac
}
...
preflight() {
  _load; _require_env
  log "▶ preflight pocket-mcp node=$NODE compose=$COMPOSE cluster=$CLUSTER"
  ...
  log "✔ preflight passed for pocket-mcp node=$NODE"
}
...
_box_run() {
  ...
  scp -o BatchMode=yes -q "$HERE/pocket-mcp-node-box.py" "$BOX_HOST:/tmp/pocket-mcp-node-box.py"
  {
    printf 'E_CHAIN_ID=%q\n' "${CHAIN_ID:-}"
    printf 'E_RPC_URL=%q\n' "$CVM_RUNTIME_RPC_URL"
    ...
    printf 'E_POCKET_API_KEY=%q\n' "${POCKET_API_KEY:-}"
    printf 'E_POCKET_API_BASE=%q\n' "${POCKET_API_BASE:-}"
    printf 'E_POCKET_API_CIDRS=%q\n' "${POCKET_API_CIDRS:-}"
    printf 'E_MCP_AUTH_TOKEN=%q\n' "${MCP_AUTH_TOKEN:-}"
    ...
    bash -c 'set -a; . /dev/stdin; set +a; exec $BOX_PY /tmp/pocket-mcp-node-box.py $mode $app_id $vm_id'"
}

_box_stop_vm() {
  local vm_id="${1:?vm_id required}"
  scp -o BatchMode=yes -q "$HERE/pocket-mcp-node-box.py" "$BOX_HOST:/tmp/pocket-mcp-node-box.py"
  ssh_box "sudo BOX_NAME='$NODE' $BOX_PY /tmp/pocket-mcp-node-box.py stop '$vm_id'"
}
...
deploy_cvm() {
  ...
  _save
  log "▶ box deploy_app pocket-mcp node=$NODE compose=$COMPOSE cluster=$CLUSTER (create stopped)"
  local out j
  ...
  _save
  log "✔ deployed pocket-mcp node app_id=$X compose_hash=$H vm=$VM_ID"
}

start_cvm() {
  _load; _require_env
  [ -n "${VM_ID:-}" ] || die "need VM_ID (run deploy first)"
  log "▶ start pocket-mcp VM vm=$VM_ID after allowlist/bind"
  ssh_box "sudo VM_ID='$VM_ID' $BOX_PY - <<'PY'
import os, sys
sys.path.insert(0, '/opt/dstack-mcp')
import mcp_dstack as m
...
PY"
  ...
  log "✔ start requested for pocket-mcp VM vm=$VM_ID"
}
...
prime_gate() {
  ...
  if [ "$(cast call "$CLUSTER" 'allowedKmsRoots(address)(bool)' "$KMS_ROOT" --rpc-url "$RPC_URL" 2>/dev/null)" != true ]; then
    send_seq "pocket-addKmsRoot-${NODE}" "$CLUSTER" "addAllowedKmsRoot(address)" "$KMS_ROOT"
  else
  ...
  if [ "$(cast call "$CLUSTER" 'allowedComposeHashes(bytes32)(bool)' "0x${H#0x}" --rpc-url "$RPC_URL" 2>/dev/null)" != true ]; then
    send_seq "pocket-addHash-${NODE}" "$CLUSTER" "addComposeHash(bytes32)" "0x${H#0x}"
  else
  ...
  if [ "$(cast call "$CLUSTER" 'allowedAppIds(address)(bool)' "$X" --rpc-url "$RPC_URL" 2>/dev/null)" != true ]; then
    send_seq "pocket-addApp-${NODE}" "$CLUSTER" "addAllowedAppId(address)" "$X"
  else
...
bind_member() {
  ...
  log "▶ bind pocket-mcp X=$X -> impl $MEMBER_IMPL (box deployer)"
  ssh_box "sudo bash -s" <<SCRIPT 2>&1 | tee "$LOGDIR/pocket-bind-${NODE}.$(ts).log"
...
SCRIPT
  confirm_latest_transaction "pocket-bind-${NODE}" "$RPC_URL" "$LOGDIR/pocket-bind-${NODE}.*.log" || die "bind transaction not confirmed"
  local c=""
  ...
  log "✔ bound pocket-mcp node X -> $CLUSTER"
}
...
verify() {
  ...
  for i in $(seq 1 45); do
    ...
    if [ -n "$id" ] && [ "$id" != "$ZERO32" ]; then
      log "✔ pocket-mcp node registered: memberId=$id memberCount=$count"
      STATE_PHASE="registered"; _save
      return 0
    fi
    log "… pocket-mcp node not registered yet ($i/45, memberCount=${count:-?})"
    sleep 20
  done
  ...
  die "pocket-mcp node did not register (owner=${owner:-?}, allowedKmsRoots[$KMS_ROOT]=${root_allowed:-?}, allowedComposeHash=${hash_allowed:-?}, allowedAppId=${app_allowed:-?})"
}
...
cleanup() {
  ...
  if [ -n "${CLUSTER:-}" ] && [ -n "${X:-}" ] && [ -n "${RPC_URL:-}" ] && [ -n "${PRIVATE_KEY:-}" ]; then
    ...
    if [ "$allowed_app" = true ] && { [ -z "$id" ] || [ "$id" = "$ZERO32" ]; }; then
      send_seq "pocket-removeApp-${NODE}" "$CLUSTER" "removeAllowedAppId(address)" "$X" || log "removeAllowedAppId failed; app remains allowlisted"
    fi
  fi
...
update_member() {
  ...
  log "✔ pocket-mcp node update complete mode=$mode vm=$VM_ID"
}
...
log "=== pocket-mcp AttestMesh node: $NODE ==="
case "$ACTION" in
  preflight) preflight ;;
  deploy) deploy_cvm ;;
  start) start_cvm ;;
  ...
  *) die "usage: pocket-mcp-node.sh <node-name> [preflight|deploy|start|prime|bind|verify|register-direct|update|cleanup|stop|all]" ;;
esac
