#!/usr/bin/env python3
"""Box-side deploy helper for deploy/clickhouse-ha-node.sh.

Runs on the self-hosted dstack box. Copy of pg-ha-node-box.py with NAME/ENV_KEYS
adjusted. CVM deployment is split so the driver can precompute every node's mesh IP
BEFORE any CVM boots:
...
The compose hash is identical for every clickhouse node: only sealed env VALUES differ per
node, and the measured allowed_envs name set is shared.
"""
...
# across every node of one cluster. VM_NAME is only the qemu/vmm display label and may vary
# per node. Splitting them keeps the compose hash the same for ch1..chN.
APP_NAME = os.environ.get("BOX_APP_NAME", "clickhouse-ha-node")
VM_NAME = os.environ.get("BOX_NAME", APP_NAME)
COMPOSE_PATH = os.environ.get("BOX_COMPOSE", "/tmp/clickhouse-ha-node.yaml")
...
# App-bound disk key: chdata/keeper-data survive fresh-disk rolls of the same app. Mesh-only
# node (gateway OFF), so the dstack-gateway instance-id restriction does not apply.
...
def main() -> None:
    ...
    if mode == "create":
        ...
        if not app_id:
            raise SystemExit("usage: clickhouse-ha-node-box.py create <app_id>")
        env = build_env()
    ...
    if mode in {"stop", "start"}:
        ...
        if not vm_id:
            raise SystemExit(f"usage: clickhouse-ha-node-box.py {mode} <vm_id>")
    ...
    if mode == "update":
        ...
        if not app_id or not vm_id:
            raise SystemExit("usage: clickhouse-ha-node-box.py update <app_id> <vm_id>")
    ...
    raise SystemExit(
        "usage: clickhouse-ha-node-box.py [register|create <app_id>|hash|update <app_id> <vm_id>|stop <vm_id>|start <vm_id>]"
    )
