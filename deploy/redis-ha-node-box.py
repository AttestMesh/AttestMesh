#!/usr/bin/env python3
"""Box-side deploy helper for deploy/redis-ha-node.sh.
...
The compose hash is identical for every redis node: only sealed env VALUES differ per
...
# per node. Splitting them keeps the compose hash the same for r1..rN.
APP_NAME = os.environ.get("BOX_APP_NAME", "redis-ha-node")
VM_NAME = os.environ.get("BOX_NAME", APP_NAME)
COMPOSE_PATH = os.environ.get("BOX_COMPOSE", "/tmp/redis-ha-node.yaml")
...
# App-bound disk key: redisdata survives fresh-disk rolls of the same app. Mesh-only
...
def main() -> None:
    ...
    if mode == "create":
        ...
        if not app_id:
            raise SystemExit("usage: redis-ha-node-box.py create <app_id>")
        env = build_env()
    ...
    if mode in {"stop", "start"}:
        ...
        if not vm_id:
            raise SystemExit(f"usage: redis-ha-node-box.py {mode} <vm_id>")
    ...
    if mode == "update":
        ...
        if not app_id or not vm_id:
            raise SystemExit("usage: redis-ha-node-box.py update <app_id> <vm_id>")
    ...
    raise SystemExit(
        "usage: redis-ha-node-box.py [register|create <app_id>|hash|update <app_id> <vm_id>|stop <vm_id>|start <vm_id>]"
    )
